    ...
    <table id="myTable" class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <input type="hidden" name="location[{{ $no }}][id]" value="{{ $data->id }}">
                    <input type="text" class="form-control" name="location[{{$no}}][location_name]" value="{{$data->location_name}}" id="">
                </td>
                <td>
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="location[{{$no}}][status]" role="switch" id="flexSwitchCheckDefault" {{$data->status ? "checked" : ""}}>
...
<script>
    ...
    function deleteConfirmation(id){
        ...
        if (result.isConfirmed) {
                console.log(fetch('{{route("location")}}/'+ id,  {
                    method: 'POST',
                    body: formData
                }));
                    Swal.fire({
...
<script>
    ...
    function myCreateFunction() {
        ...
        var location_name = row.insertCell(1);
        ...
        location_name.innerHTML =
            ...
            count +
            "][id]' value=''><input type='text' class='form-control' name = 'location[" +
            count +
            "][location_name]' value = '' placeholder = 'Nama Lokasi'> ";
        status.innerHTML = "<div class='form-check form-switch'> <input class='form-check-input' type='checkbox' name='location[" +
            count +
